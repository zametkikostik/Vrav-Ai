...
- JSON request body ограничен 1 MiB; некорректный `Content-Length` возвращает `400`, а превышение лимита — `413`.
- JSON endpoints требуют `Content-Type: application/json` (иначе `415 unsupported_media_type`).
- `session_id` в `POST /stream` должен быть непустой строкой длиной до 128 символов; значение нормализуется через trim.
